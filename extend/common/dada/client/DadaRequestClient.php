 ...
 * 达达接口请求客户端
 */
namespace common\dada\client;
use common\dada\client\DadaResponse;
use common\dada\config\DadaConstant;
...
class DadaRequestClient{
    ...
    public function parseResponseData($arr){
        ...
        if (empty($arr)) {
            ...
            isset($data['status'])?$resp->setStatus($data['status']):'';
            isset($data['msg'])?$resp->setMsg($data['msg']):'';
            isset($data['code'])?$resp->setCode($data['code']):'';
            isset($data['result'])?$resp->setResult($data['result']):'';
        }
        return $resp;
    }
