...
//			level: "level",			//层级
	...
	DTree.prototype.getLiItemDom =  function(treeId, parentId, title, isLast, iconClass, checkArr, level, spread, disabled, basicData, recordData, flag) {
		...
		var li = ["<li " + "class='"+LI_CLICK_CHECKBAR+" "+ LI_NAV_ITEM +"'" + "data-id='"+treeId+"'" + "data-pid='"+(flag == "root" ? (parentId ? parentId : "-1") : parentId)+"'" + "dtree-id='"+rootId+"'" + "data-admin='"+level+"'" + ">" +
		          	div ,
					dom.fnode(),
					dom.node(),
					dom.checkbox(),
	...
	DTree.prototype.getCheckbarNodeParam = function($div, $i){
		...
		temp_node.level = $div.parent().attr("data-admin");
	...
	DTree.prototype.setNodeParam = function($div) {
		...
		_this.node.level = $div.parent().attr("data-admin");
	...
	DTree.prototype.bindBrowserEvent = function(){
		...
		_this.obj.on("click", "i[data-spread]", function(event) {
			...
			var $i = $(this),
				...
				node = _this.getNodeParam($div),
				$ul = $div.next("ul"),
				$p_li = $div.parent("li[data-admin]"),	//当前选中节点的顶级li节点
				$p_ul = $p_li.parent("ul");
						...
						success: function(layero, index){
							...
							form.on("submit(dtree_addNode_form)",function(data){
								...
								var parentId = $div.attr("data-id"),
									...
									isLeaf = true,
									isChecked = "0",
									level = parseInt($p_li.attr("data-admin"))+1;
