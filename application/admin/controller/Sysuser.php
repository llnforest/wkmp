...
class Sysuser extends BaseController
{
    ...
    {
        parent::__construct($app,SysUserModel::class);
    }
    ...
    public function resetPassword(){
        if($this->request->isPost()){
            ...
            $this->param['password'] = md5(md5(SysConfigModel::getValue('resertPassword')));
    ...
    function beforeDel($data){
        if($this->request->isPost()){
            SysUserRoleModel::where(['user_id' => $this->param['id']])->delete();
        }
    }
    ...
    private function addUserRole($user_id){
        if($user_id) SysUserRoleModel::where(['user_id'=>$user_id])->delete(); //删除
